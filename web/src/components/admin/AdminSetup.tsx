...
const Toggle = ({ checked, onChange, label, hint }: { checked: boolean; onChange: (v: boolean) => void; label: string; hint: string }) => (
    <button type="button" onClick={() => onChange(!checked)} className="w-full flex items-center justify-between gap-4 text-left p-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700">
        ...
        <span className={`relative shrink-0 w-11 h-6 rounded-full transition-colors ${checked ? 'bg-emerald-600' : 'bg-slate-300 dark:bg-slate-600'}`}>
        ...
        <main className="flex items-center justify-center min-h-screen bg-slate-100 dark:bg-slate-900 p-4">
            <div className="w-full max-w-md p-8 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700">
                <div className="flex flex-col items-center mb-6">
                    <div className="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mb-4 text-emerald-600 dark:text-blue-400">
                        <ShieldCheck size={32} />
                    </div>
                    ...
                    <div className="flex gap-1.5 mt-3">
                        <span className={`h-1.5 w-10 rounded-full ${step >= 1 ? 'bg-emerald-600' : 'bg-slate-200 dark:bg-slate-700'}`} />
                        <span className={`h-1.5 w-10 rounded-full ${step >= 2 ? 'bg-emerald-600' : 'bg-slate-200 dark:bg-slate-700'}`} />
                    </div>
                </div>
                        ...
                        <>
                            <label className="block text-sm font-bold text-slate-600 dark:text-slate-300">
                                ...
                                <span className="relative block mt-1">
                                    ...
                                    <input type="text" value={serverName} onChange={(e) => setServerName(e.target.value)} placeholder="La meva llista" className="w-full min-h-11 pl-10 pr-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white" />
                                </span>
                            </label>
                            ...
                            <button type="button" onClick={() => setStep(2)} className="w-full min-h-11 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold flex items-center justify-center gap-2">
                        ...
                        <>
                            <label className="block text-sm font-bold text-slate-600 dark:text-slate-300">
                                ...
                                <span className="relative block mt-1">
                                    ...
                                    <input type="email" autoComplete="username" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full min-h-11 pl-10 pr-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white" />
                                </span>
                            </label>
                            <label className="block text-sm font-bold text-slate-600 dark:text-slate-300">
                                Contrasenya
                            ...
                            <label className="block text-sm font-bold text-slate-600 dark:text-slate-300">
                                ...
                                <span className="relative block mt-1">
                                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                                    <input type="password" autoComplete="new-password" required value={confirm} onChange={(e) => setConfirm(e.target.value)} className="w-full min-h-11 pl-10 pr-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white" />
                                </span>
                            </label>
                            ...
                            <div className="flex gap-2">
                                ...
                                <button type="submit" disabled={loading} className="flex-1 min-h-11 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold disabled:opacity-50 flex items-center justify-center gap-2">
